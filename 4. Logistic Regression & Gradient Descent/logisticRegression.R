...
#
# The error measure for logistic regression is based on likelihood.  It is known as "cross-entropy error" and takes the
# following form:
#
# E_in(w) = (1/N)*SIGMA(n = 1, N) ln(1 + e^(-yn*t(w)%*%xn))
#
# It measures how likely we are to generate y from x under the assumption that h = f; in other words, it measures the 
# probability of the data given the hypothesis.  Maximizing the likelihood translates into minimizing an error measure.  
# Unlike the error measure for linear regression, there is no closed form solution for the minimum of the error measure 
# for logistic regression; thus, an iterative solution is used instead.  
#
...
# theta(s) = e^(s)/(1 + e^(s))
#
# E(w) = (1/N)*SIGMA(n = 1, N) ln(1 + e^(-yn*t(w)%*%xn))  # cross-entropy error
#
